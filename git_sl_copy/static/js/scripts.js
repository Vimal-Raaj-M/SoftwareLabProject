...
function renderEventsList(events, listId = 'eventsList') {
    ...
    events.forEach(event => {
        ...
        li.innerHTML = `
            ...
            ${listId === 'eventsList' ? 
                `<button onclick="addToMyEvents('${event.event_id}','Public')" class="favorite-btn">Add Event</button>` : 
                `<button onclick="removeFromEvents('${event.event_id}', '${listId}')" class="favorite-btn">Remove</button>`
            }
        `;
        list.appendChild(li);
    });
